...
interface FileTypeSummary {
  size: number;
  latestDate: string; // или Date, если вы работаете с объектами Date
}
...
interface UsageSummaryItem {
  title: string;
  size: number;
  latestDate: string; // или Date
  icon: string;
  url: string;
}

export const getUsageSummary = (totalSpace: TotalSpaceUsed): UsageSummaryItem[] => {
  return [
    {
      title: "Documents",
