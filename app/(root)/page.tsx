  ...
  return (
    <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 md:grid-cols-2 xl:gap-10">
      ...
      <section className="h-full rounded-[20px] bg-white p-5 xl:p-8">
        <h2 className="h3 xl:h2 text-light-[#333F4E]">Recent files uploaded</h2>
        {files.documents.length > 0 ? (
          <ul className="mt-5 flex flex-col gap-5">
            {files.documents.map((file: Models.Document) => (
              <Link
              //@ts-expect-error csdc
                href={file.url}
                target="_blank"
              ...
              >
                <Thumbnail
                //@ts-expect-error sdcscds
                  type={file.type}
                  //@ts-expect-error fcsdcsc
                  extension={file.extension}
                  //@ts-expect-error scsdc
                  url={file.url}
                />
