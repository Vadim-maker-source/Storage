...
import Thumbnale from './Thumbnail'
...
const FileUploader = ({ ownerId, accountId, className }: Props) => {
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        const uploadPromises = acceptedFiles.map(async (file) => {
            ...
            const toastText1 = <p className="text-[15px] leading-[20px] font-normal text-[#FF7474]"><span className="font-semibold">{file.name}</span> is too large. Max file size is 50MB.</p>
            return toast(toastText1)
            }
      ...
      return (
        <div {...getRootProps()} className="cursor-pointer">
          <input {...getInputProps()} />
          <Button type="button" className={cn("bg-[#FA7275] h-[52px] gap-2 px-10 shadow-drop-1", className)}>
            ...
            {files.length > 0 && (
                <ul className="fixed bottom-10 right-10 z-50 flex size-full h-fit max-w-[480px] flex-col gap-3 rounded-[20px] bg-white p-7 shadow-drop-3">
                    <h4 className="text-[18px] leading-[20px] font-medium text-[#333F4E]">Uploading</h4>
                        ...
                        return (
                            <li key={`${file.name}-${index}`} className="flex items-center justify-between gap-3 rounded-xl p-3 shadow-drop-3">
                                <div className="flex items-center gap-3">
                                    <Thumbnale type={type} extension={extension} url={convertFileToUrl(file)} />
