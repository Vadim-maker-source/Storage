    ...
    return (
        <div className="relative w-full md:max-w-[480px]">
            <div className="flex h-[52px] flex-1 items-center gap-3 rounded-full px-4 shadow-drop-3">
                ...
                <Input value={query} placeholder="Search..." className="text-[14px] leading-[20px] font-normal shad-no-focus placeholder:body-1 w-full border-none p-0 shadow-none placeholder:text-[#A3B2C7]" onChange={(e) => setQuery(e.target.value)} />
                {open && (
                                ...
                                <li key={file.$id} className="flex items-center justify-between" onClick={() => handleClickItem(file)}>
                                   <div className="flex cursor-pointer items-center gap-4">
                                    {/* @ts-expect-error - Временное решение, если name действительно есть */}
                                        ...
                                        {/* @ts-expect-error - Временное решение, если name действительно есть */}
                                        <p className="text-[14px] leading-[20px] font-semibold line-clamp-1 text-[#333F4E]">{file.name}</p>
                                    </div>

                                    <FormattedDateTime date={file.$createdAt} className="text-[12px] leading-[16px] font-normal line-clamp-1 text-[#A3B2C7]" />
